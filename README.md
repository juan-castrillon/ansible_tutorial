...
Roles can also be used in a task with `import_role`. When running with `roles` ansible will execute the roles after any `pre_task` but before any ohter task in the play. When calling it from a task with `import_role` the order is respected. 

Most importantly, `import_roles` is dynamic, which alows to do things like have variables for the role name, use loops to execute them, skip them. 
...
> If not using the `roles` directory, a path can be passed instead of just the name
...
- Inside each role folder, the following structure is standard:
  ...
  - `vars` to put high precendece variables provided by the role  with the default one always being `main.yml`
  - `files` to put any files referenced in the role tasks (like in `copy`)
  - `templates` to put any role templates i
